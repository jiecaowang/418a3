...
void PointLight::shade( Ray3D& ray ) {
	...
	if (!ray.intersection.none){
		...
		Vector3D view = Vector3D(-ray.intersection.point[0], -ray.intersection.point[1], -ray.intersection.point[0]);
		Vector3D incident = -1 * ray.dir;
		Vector3D H = view + incident;
		H.normalize();

		Colour ambient = _col_ambient * ray.intersection.mat->ambient;
		Colour diffuse = (ray.intersection.normal.dot(-ray.dir)) * (_col_diffuse * ray.intersection.mat->diffuse);
		diffuse.clamp();
